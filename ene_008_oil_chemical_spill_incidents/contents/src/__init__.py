...
def get_date_1m(title, new_date):
    '''
    ...
    old_date_text = title.split(' U.S. Oil')[0]
...
def get_date(title, new_date):
    ...
    '''
    # get current end date being used from title by string manupulation
    old_date_text = title.split(' U.S. Oil')[0].split(' - ')[1]
    # get text for new date
...
def update_layer(layer, new_date):
    ...
    # if we are processing the layer that shows Oil and Chemical Spills from 1957 to the present
    if lyr_dscrptn.endswith('1957 to the present.'):
        old_date_text, new_date_text = get_date(cur_title, new_date)
    # if we are processing the layer that shows Oil and Chemical Spills for latest 30 days
    else:
        old_date_text, new_date_text = get_date_1m(cur_title, new_date)
...
def updateResourceWatch(num_new):
    ...
    if num_new>0:
        ...
        for layer in layer_dict:
            # no need to add dates to the layer that shows only 2018 data
            if layer['id'] != '83ecd845-ac63-45b4-b795-63b2ce934d12':
